...
def generate_launch_description():
    ...
    filepath_config_twist_mux = PathJoinSubstitution(
        [FindPackageShare('bunker_control'), 'config', 'twist_mux.yaml']
    )

    filepath_config_interactive_markers = PathJoinSubstitution(
        [FindPackageShare('bunker_control'), 'config', 'teleop_interactive_markers.yaml']
    )
    ...
    node_twist_mux = Node(
        ...
        output='screen',
        remappings={('/cmd_vel_out', '/bunker_velocity_controller/cmd_vel_unstamped')},
